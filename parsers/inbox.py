...
class InboxParser(Parser):
    ...
    def process(self, year: Optional[int]) -> None:
        ...
        user_directories = list(self.directory_to_message_data.keys())
    ...
    def print(self, number=10) -> None:
        print('You messaged a total of {0} people'.format(len(list(self.user_directory_to_message_types.keys()))))

        def get_total(key):
            total = 0
            for user_directory in list(self.directory_to_message_data.keys()):
                total += len(self.user_directory_to_message_types.get(user_directory, {}).get(key, []))
            return total
        ...
        def get_users_by_message_frequency(key, users):
            users.sort(key=lambda user_directory: len(self.user_directory_to_message_types.get(user_directory, {}).get(key, [])), reverse=True)
            return users

        for key in ['texts', 'photos', 'videos']:
            print('You exchanged the most {0} with these people'.format(key))
            users = get_users_by_message_frequency(key, list(self.directory_to_message_data.keys()))
            for user in users[:number]:
                print('  {0} {1} {2}'.format('_'.join(user.split('_')[0:-1]), len(self.user_directory_to_message_types.get(user, {}).get(key, [])), key))
            print('')
        ...
        def get_users_by_message_disparity(your_username, users, sort_by_user_sent):
            ...
            valid_users = []
            for user in users:
                texts = self.user_directory_to_message_types.get(user, {}).get('texts', [])
                total = len(texts)
                if total < 1:
                    continue
        ...
        users_by_message_disparity, user_to_percentage_you_sent = get_users_by_message_disparity(self.username, list(self.directory_to_message_data.keys()), True)
        ...
        users_by_message_disparity, user_to_percentage_you_sent = get_users_by_message_disparity(self.username, list(self.directory_to_message_data.keys()), False)
        for user in users_by_message_disparity[:number]:
